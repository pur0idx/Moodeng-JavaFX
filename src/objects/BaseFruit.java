...
public abstract class BaseFruit extends ImageView implements Collectible, Temporal, PowerUpEffect {
	...
	public BaseFruit(String name, String imageFile, double duration) {
		this.name = name;
	...
	protected void setupImage() {
		try {
			Image image = new Image(ClassLoader.getSystemResource(imageFile).toString(), 32, 0, true, false);
			setImage(image);
		} catch (Exception e) {
	...
	protected void setupTimer() {
		if (duration > 0) {
			effectTimer = new Timeline(new KeyFrame(Duration.seconds(duration), e -> {
				isActive = false;
